...
def run_interface():
    # Hàm đọc và hiển thị dữ liệu trong Treeview
    ...
    # Hàm hiển thị dữ liệu trong TreeView theo trang
    def display_page():
        nonlocal current_page, total_pages, df_data
        tree.delete(*tree.get_children())  # Xóa dữ liệu cũ trên giao diện

        start_idx = (current_page - 1) * items_per_page  # Dòng bắt đầu của trang hiện tại
        end_idx = start_idx + items_per_page  # Dòng kết thúc của trang hiện tại

    # Duyệt qua các dòng và thêm số thứ tự
        for idx, (_, row) in enumerate(df_data.iloc[start_idx:end_idx].iterrows(), start=start_idx + 1):
            tree.insert("", "end", values=[idx] + list(row))  # Thêm số thứ tự vào cột đầu tiên
    ...
    # Hàm đọc dữ liệu và hiển thị trang đầu tiên
    def read_data():
       nonlocal current_page, total_pages, df_data
    ...
    def go_to_previous_page():
       ...
       if current_page > 1:
        current_page -= 1
        display_page()  # Hiển thị trang mới
    ...
    def prev_page():
        ...
        if current_page > 1:
            current_page -= 1
            display_page(df_data, current_page)
            update_pagination_buttons()
...
# Các nút chuyển trang
    btn_prev = tk.Button(frame_pagination, text="Trang trước", command=go_to_previous_page)
    btn_prev.pack(side="left", padx=5)

    btn_next = tk.Button(frame_pagination, text="Trang sau", command=go_to_next_page)
