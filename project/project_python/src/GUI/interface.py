...
def run_interface():
    ...
    button_frame.pack(fill="y", side="left", padx=10, pady=10)
    ...
    for col in columns:
        tree.heading(col, text=col)
        tree.column(col, width=120, anchor='center')  # Giảm chiều rộng cột để phù hợp hơn
    ...
    button_list = [
        ("Tải Dữ Liệu", read_data),
        ("Thêm Laptop", open_create_window),
        ("Cập Nhật Laptop", open_update_window),
        ("Làm Sạch Dữ Liệu", clean),
        ("Chuẩn Hóa Dữ Liệu", normal),
        ("Vẽ Biểu Đồ", open_chart_window),
    ]

    # Tạo và bố trí các nút dọc
